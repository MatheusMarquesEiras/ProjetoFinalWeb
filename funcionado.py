    ...
    if request.method == 'POST':
        ...
        if new_conversation:
            try:
                ...
                # Retorna os dados para serem usados no GET
                return jsonify({"message": "Text received, SSE ready", "conversation_id": new_conv.id, "text_user": text_user}), 200
        ...
        return jsonify({"message": "Text received, SSE ready"}), 200

    elif request.method == 'GET':
        # Recupera os parâmetros da URL
        text_user = request.args.get('text_user')
        Logger.info(f'texto recebido:  {text_user}')
        conversation_id = request.args.get('conversation_id')

        if not text_user or not conversation_id:
            return jsonify({"error": "Missing required parameters"}), 400

        Logger.info(f"GET /gen - Streaming for text: {text_user}")

        try:
            stream = Server.stream_server_request_response(text=text_user)
            ...
            def generate():
                ...
                try:
                    ...
                    # Adiciona o contexto manualmente ao banco de dados
            ...
            Logger.error(f"GET /gen - Error occurred for text_user: {text_user}, error: {str(e)}")
...
def get_titles():
    ...
    try:
        ...
        user_convs = Conversation.query.filter_by(user_id=session['user_id']).order_by(desc(Conversation.created_at)).all()
...
def get_history():
    data = request.json
    try:
        history = Phase.query.with_entities(Phase.content).filter_by(conversation_id=data['conv_id']).all().order_by(Phase.created_at)
        history_list = [item.content for item in history]
